...
module.exports = class Scripts {
	...
	static checkTemplates(config, rootDir) {
		config.drivers.forEach(driver => {
			if (!driver.id) return console.error(`Driver.id cannot be ${driver.id} for driver config ${JSON.stringify(driver, null, '\t')}`);
			if (!driver.pair) return;
			if (!driver.id) throw new Error('Driver id must be a string');
	...
	static copyDriverTemplate(driver, templateId, template, templatePath, templateLock) {
			...
			if (templateLock) {
				...
				if (versionBump) {
					if (currTemplateString) {
						...
						if (templateLock.hash && crypto.createHash('md5').update(currTemplateHashString[1]).digest('hex') !== templateLock.hash) {
							console.log(`Skipping ${driver.id}:${templateId} template update from ${templateLock.version} to ${template.version} (${versionBump} version) because the template contains manual changes!`);
							return;
						}
					}
				} else {
				...
				console.log(`Updating ${driver.id}:${templateId} template from ${templateLock.version} to ${template.version} (${versionBump} version).`);
			}
			const currTemplateHashString = currTemplateString.match(parseTemplateRegex);
			if (!(currTemplateHashString && currTemplateHashString[1])) {
				console.log(`Template ${driver.id}:${templateId} corrupt and does not contain START and END template indicators.`);
				return;
			}
			if(!templateLock) {
				console.log(`Updating ${driver.id}:${templateId} template to ${template.version}`);
			}

			const oldTemplateStringIndex = currTemplateString.indexOf(currTemplateHashString[1]);
			const newTemplate = currTemplateString.slice(0, oldTemplateStringIndex) + templateHashString[1] + currTemplateString.slice(oldTemplateStringIndex + currTemplateHashString[1].length);
			fse.writeFileSync(templatePath, newTemplate);
			return {
				version: template.version,
				hash: templateChecksum,
			}
		}
		console.log(`Generating ${driver.id}:${templateId} template version ${template.version}`);
