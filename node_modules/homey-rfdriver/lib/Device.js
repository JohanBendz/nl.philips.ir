...
module.exports = superclass =>
	...
	class RFDevice extends superclass {
		...
		constructor(...args) {
			...
			console.log('capabilityMap', this.commandToCapabilityMap);
		...
		sendProgramSignal(data = {}) {
			const updateState = (data, frame, dataCheck) => {
				dataCheck = this.parseIncomingData(dataCheck);
				if (!dataCheck) return;

				this.emit('data', dataCheck);
			};
			this.once('send', updateState);

			return this.send(data)
				.catch(() => this.removeListener('send', updateState));
		}
		...
		matchesCmd(cmdObj) {
			return ['type', 'subtype']
				.every((key) => {
					...
					const data = this.getData();
					const configValue = data.hasOwnProperty(configKey) ? data[configkey] : this.options[configKey];
		...
		getCmds() {
			if (!this.cmdCache) {
				...
				const cmdType = typeDef.hasOwnProperty('cmdType') ? typeDef.cmdType : this.options.cmdType;
				const cmdSubType = typeDef.hasOwnProperty('cmdSubType') ? typeDef.cmdSubType : this.options.cmdSubType;
		...
		/**
		 * Default flow implementation function for ${driverId}:received. Checks if keys in args match the keys in device state.
		 * @param args
		 * @param state
		...
		onFlowTriggerFrameReceived(args, state) {
			return Object.keys(args).reduce(
				(result, key) => result && (key === 'device' || String(args[key]) === String(state[key])),
				true
			);
		}

		/**
		 * Default flow implementation function for ${driverId}:send. Sends the args object.
		 * @param args
		 * @returns {Promise} result
		 */
