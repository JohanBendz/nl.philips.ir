...
class RFDriver extends Homey.Driver {
	...
	initDefaultFlows(skip = []) {
		...
		if (!this.flowTriggerDeviceFrameReceived || skip.includes['onFlowTriggerFrameReceived']) {
			const receivedTrigger = this.getFlowTriggersManifest({ filterFn: (card) => card.id === `${this.id}:received` });
		...
		if (!this.flowActionDeviceFrameSend || skip.includes['onFlowActionFrameSend']) {
			const sendAction = this.getFlowActionsManifest({ filterFn: (card) => card.id === `${this.id}:send` });
		...
		// TODO rewrite default flow cards
		...
		// TODO: implement default send cmd action card implementation
		// if (this.manifest.actions) {
		// 	const sendCmdActionConfig = this.manifest.actions.find(actions => actions.id === `${this.id}:send_cmd`);
		// 	if (sendCmdActionConfig) {
		// 		this.sendCmdAction = new Homey.FlowCardAction(`${this.id}:send_cmd`)
		// 			.register()
		...
		// 		});
		// 		if ((sendCmdActionConfig.args.find(arg => arg.name === 'cmd') || {}).type === 'autocomplete') {
	...
	genericRunListener(triggerFnName, args, state) {
		...
		return device[triggerFnName](Object.assign({ device }, args), state);
	}
	...
	onPair(socket) { // Pair sequence
		...
		socket.on('set_device', ({ data, settings, store, capabilities }, callback) => {
			...
			if (!pairDevice.assembleDeviceObject()) {
				return callback(this.translateError(new Error('invalid_device_data')));
			}
		...
		socket.on('test', (data, callback) => {
			...
			if (!pairDevice.assembleDeviceObject()) return callback(this.translateError(new Error('pairdevice_not_complete')));
			callback(null, pairDevice.deviceState);
		});
		...
		socket.on('done', (data, callback) => {
			...
			if (!deviceObject) {
				const err = new Error('Invalid device object from Device.assembleDeviceObject() on pair wizard done event!');
				this.error(err);
				return callback(err);
			}
