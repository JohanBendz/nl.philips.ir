...
class Signal extends EventEmitter {
            ...
            .then(() => {
                ...
                if (this.minTxInterval) {
                    if ((Date.now() - this.lastTx) < this.minTxInterval) {
                        ...
                        return new Promise(res => setTimeout(() => res(sendTx), this.lastTx - Date.now()));
                    }
    ...
    sendCmd(cmd) {
        ...
        return this.register(registerLockKey).then(() => {
            ...
            return new Promise((resolve, reject) => {
                const send = () => this.signal.cmd(sendCmd, (err, result) => { // Send the cmd to device
    ...
    getCmdLabel(cmdObj) {
        ...
        if (!result && cmdObj.type && cmdObj.subType) {
            const key = `rf.commands.${cmdObj.type}.${cmdObj.cmd}.${cmdObj.subType}`;
            result = Homey.__(key);
            result = result === key ? null : result;
        }
        if (!result && cmdObj.type) {
            const defaultKey = `rf.commands.${cmdObj.type}.${cmdObj.cmd}.default`;
            result = Homey.__(defaultKey);
            result = result === defaultKey ? null : result;
            if (!result) {
                const key = `rf.commands.${cmdObj.type}.${cmdObj.cmd}`;
                result = Homey.__(key);
                result = result === key ? null : result;
            }
        }
        if (!result && cmdObj.subType) {
            const key = `rf.commands.${cmdObj.cmd}.${cmdObj.subType}`;
            result = Homey.__(key);
            result = result === key ? null : result;
        }
        if (!result) {
            const defaultKey = `rf.commands.${cmdObj.cmd}.default`;
            result = Homey.__(defaultKey);
            result = result === defaultKey ? null : result;
        }
        if (!result) {
            const key = `rf.commands.${cmdObj.cmd}`;
            result = Homey.__(key);
            result = result === key ? null : result;
        }
        if (!result) {
